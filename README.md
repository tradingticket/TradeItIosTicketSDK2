...
```swift
let authInfo = TradeItAuthenticationInfo(id: "dummy",
                                         andPassword: "pass",
                                         andBroker: "dummy")

TradeItLauncher.linkedBrokerManager.linkBroker(authInfo: authInfo,
...
TradeItLauncher.linkedBrokerManager.authenticateAll(onSecurityQuestion: { securityQuestion, answerSecurityQuestion in
    TradeItAlert().show(securityQuestion: securityQuestion,
                        onViewController: self,
                        onAnswerSecurityQuestion: answerSecurityQuestion)
}, onFinished: {
    TradeItAlert().showErrorAlert(onViewController: self,
                                  title: "authenticateAll finished",
                                  message# TradeIt iOS Ticket SDK 
...
TradeItLauncher.linkedBrokerManager.linkBroker(authInfo: authInfo,
                                               ...
                                               onFailure: { errorResult in
                                                   ...
                                                   // TradeItAlertManager can be used to display a UIAlertController on the current screen
                                                   TradeItAlertManager().showError(errorResult, onViewController: self)
                                               }
```
...
```swift
TradeItLauncher.linkedBrokerManager.authenticateAll(onSecurityQuestion: { securityQuestion, answerSecurityQuestion in
    // TradeItAlertManager can be used to handle security questions
    TradeItAlertManager().promptUserToAnswerSecurityQuestion(
        securityQuestion,
        onViewController: self,
        onAnswerSecurityQuestion: answerSecurityQuestion,
        onCancelSecurityQuestion: { print(":(") } )
}, onFinished: {
...
: "\(TradeItLauncher.linkedBrokerManager.linkedBrokers.count) brokers authenticated.")
})
```
